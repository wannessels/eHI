...
Native sealing is a single-pass pipeline: plaintext flows through inner hashing/signing, AES encryption and outer hashing/signing into the final result. Non-seekable input is not spooled or replayed, including during an outer-signature retry. Output uses standard chunked BER CMS. Unsealing validates the outer signature before choosing a decryption certificate, then feeds decrypted bytes directly into inner verification and final output; it retains an encrypted-envelope spool to preserve historical certificate selection by signing time.

Each retained native stage/output spills to a temporary file above `Settings.Default.InMemorySize` (1 MiB by default), with known large stages starting on disk. Payload copies and native temporary-file I/O are asynchronous; small ASN.1 framing/metadata operations and cryptographic primitives still run synchronously. Caller streams stay open, returned streams belong to the caller, and failed/cancelled operations dispose their temporary streams. Completed unseal results retain the existing security-status API: callers must check validation/trust status before using plaintext.
...
See [performance configuration](performance.md), [current native streaming measurements](native-streaming-performance.md), and [benchmark methodology](../benchmarks/README.md). The earlier [buffered memory improvements](native-memory-improvements.md), [backend comparison](backend-crypto-performance.md), [native migration comparison](native-crypto-performance.md), and [profiling report](performance-profiling.md) are historical snapshots.
