...
Each native intermediate/output stream spills to a temporary file above `Settings.Default.InMemorySize` (1 MiB by default). Known large stages start directly on disk. Sealing non-seekable input first spools it because signing and embedding require separate passes; verification/unsealing reads non-seekable inputs incrementally. Caller streams stay open, returned streams belong to the caller, and failed/cancelled operations dispose their temporary streams. Completed unseal results retain the existing security-status API: callers must check validation/trust status before using plaintext.
