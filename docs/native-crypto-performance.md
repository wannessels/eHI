...
This historical snapshot measured the migration that removed BouncyCastle completely. A [selectable streaming backend](native-crypto-migration.md#backend-selection) has since been restored; see the [current backend comparison](backend-crypto-performance.md). The original migration was **not an unqualified performance win**: native CMS/AES was faster for larger messages, but the platform CMS API allocated more memory than the previous streaming implementation.
