...
The libraries support .NET Framework 4.6.2, .NET Standard 2.0, .NET 6 and .NET 8. Set `Settings.Default.UseNativeCrypto = true` for native message cryptography (the default), or `false` for BouncyCastle. Both backends stream large payloads through temporary files. Older runtimes use compatibility implementations for missing metadata/stream APIs, while retaining shared timestamp and revocation policy. See [runtime support and CI](runtime-support.md) and the [migration guide](native-crypto-migration.md) for exact scope, factory overrides and key-provider requirements.

The [review follow-up](performance-review-follow-up.md) documents the trust-cache and recipient-rotation fixes, restored null-level revocation, the 16 MiB spill default, and explicit PKCS#1 signing for non-exportable keys. RSA signing defaults to PSS; configure `Settings.Default.RsaSignaturePadding` or a sealer factory override to select PKCS#1 when the provider and receiving profile require it.
...
```csharp
Settings.Default.InMemorySize = 64L * 1024 * 1024; // raise the spill threshold from the 16 MiB default to 64 MiB
...
Service clients reuse crypto contexts. Replacing the signing certificate, current store or expired-store set, or changing `UseNativeCrypto` or `RsaSignaturePadding`, retires the corresponding context after its last active operation. Directly held factory-created contexts retain their original backend and signing padding; create replacements to switch them. Close or abort clients when finished. Certificates/stores remain owned by the caller and must outlive active operations. Configure these properties before sharing a client; do not mutate its store list concurrently.
...
Successful chain builds using explicit `X509CertificateHelper.CustomTrustStore` anchors are cached by the leaf, extra-store and trust-anchor thumbprints and the download policy for `ChainCache.Lifetime` (ten minutes by default). Zero disables lookup and insertion and clears existing entries. Paths are reused only while every certificate is valid at the requested time; a path with any error status is rebuilt. System-trust paths (`CustomTrustStore = null`) are rebuilt by the platform on every call to avoid caching a stale trust decision; this costs more than a cache hit. Revocation is checked separately, including at null verification/unsealing level. Trust-store assignment, download-policy changes and `ChainCache.Clear()` replace the cache generation so earlier builds cannot repopulate it. `Count` and `EntryLimit` (1024) are exposed. Serialize in-place trust collection edits with validation; operations already running use their captured configuration.
