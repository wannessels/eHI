...
{
    private static X509Certificate2 Certificate(RSA rsa) => new CertificateRequest("CN=WS-Security", rsa, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1)
    ...
    {
        int opened = 0;
        using var pool = new SigningKeyPool(() => { Interlocked.Increment(ref opened); return RSA.Create(2048); }, 2);
