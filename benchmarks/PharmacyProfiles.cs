...
internal static class PharmacyProfiles
{
    ...
    internal static async Task<object> RunAsync(int concurrency, int requests, int prescribers, int citizenCrlEntries, int ehealthCrlEntries, bool native)
    {
        ...
        RevocationCache.Clear(); ChainCache.Clear(); CertificateCache.Clear();
        ...
        try
        {
            ...
            var rounds = await ClosedLoop.RunAsync(concurrency, requests, Request,
                () => new { CrlDownloads = Volatile.Read(ref pki.CrlDownloads), RevocationCacheEntries = RevocationCache.Count, RevocationCacheEstimatedBytes = RevocationCache.EstimatedSizeBytes, ChainCacheEntries = ChainCache.Count, ChainCacheHits = ChainCache.Hits, ChainCacheMisses = ChainCache.Misses, CertificateCacheEntries = CertificateCache.Count });
            return new
            {
