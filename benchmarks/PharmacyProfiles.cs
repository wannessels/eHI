    ...
    internal static async Task<object> RunAsync(int concurrency, int requests, int prescribers, int citizenCrlEntries, int ehealthCrlEntries, bool native)
    {
        ...
        try
        {
            ...
            var rounds = await ClosedLoop.RunAsync(concurrency, requests, Request,
                () => new { CrlDownloads = Volatile.Read(ref pki.CrlDownloads), RevocationCacheEntries = RevocationCache.Count, RevocationCacheEstimatedBytes = RevocationCache.EstimatedSizeBytes, ChainCacheEntries = ChainCache.Count, ChainCacheHits = ChainCache.Hits, ChainCacheMisses = ChainCache.Misses, CertificateCacheEntries = CertificateCache.Count, PublicKeyCacheEntries = PublicKeyCache.Count });
            return new
            {
