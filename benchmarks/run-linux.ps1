param(
    [ValidateSet('all', 'crypto', 'memory', 'http', 'native-memory', 'crypto-concurrency', 'pharmacy')][string]$Suite = 'all',
...
if ($Suite -eq 'pharmacy') { $profileName = "pharmacy-c$Concurrency-p$Prescribers-crl$CitizenCrlEntries-net8-$($CpuLimit)cpu-$($MemoryGiB)g" + $(if ($DisableTiering) { '-no-tiering' } else { '' }) }
...
if ($Suite -eq 'pharmacy') { $profileArguments += " --concurrency $Concurrency --requests $Requests --prescribers $Prescribers --citizen-crl-entries $CitizenCrlEntries --ehealth-crl-entries $EHealthCrlEntries" }
