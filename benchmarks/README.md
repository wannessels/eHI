...
- Signing: 200 operations per round, RSA-2048/3072, 64 B/256 KiB input. Key generation/export and factory creation happen outside measurement. The platform case uses RSA.SignData with PSS; it includes hashing plus signing. BouncyCastle is a benchmark-only comparison dependency. Generated signatures are checked before measurement.
- CMS: production B-level seal/unseal with a reused RSA-2048 WebKey signer and AES-128 shared recipient key. Payloads are 32 KiB, 1 MiB, 1 MiB + 1 B and 8 MiB. Each round uses 40 operations through 1 MiB and 12 above it. Signature validity, decoded length and the final payload byte are checked. No certificate revocation, STS/TSA calls or SOAP transport are part of this case. CMS scenarios measure the production platform implementation; older committed results retain the former partial-migration variants. The alternative memory threshold is 64 MiB versus the 1 MiB default.
