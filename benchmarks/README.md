...
The executable also accepts `--suite crypto|memory|http|native-memory|all`, `--quick`, and `--output path`. Run `-Suite native-memory` in a fresh container/process for an isolated 8 MiB native CMS round trip: its process peak working set is not contaminated by earlier payload sizes or the BouncyCastle cases. It measures single-operation concurrency, not a concurrency sizing recommendation.
